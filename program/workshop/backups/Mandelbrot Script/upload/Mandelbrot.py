...
class MandelbrotEvent:
	...
	def initializePalette(self , artboard):	
		i = 0
		colorArray = ScriptingUtils.bytes(4)		
		colorArray[3] = -1
		
		while i < self.iterations:
			colorArray[0] = ScriptingUtils.doubleToByte(Math.abs(Math.pow(i , i)))
			colorArray[1] = ScriptingUtils.doubleToByte(Math.abs(Math.pow(i , 2)))
			colorArray[2] = ScriptingUtils.doubleToByte(Math.abs(Math.pow(i , 3)))
