...
public final class NumberUtils {
	...
	public static int nearestPowerOfTwo(int nearestToThis) {
		...
		assert nearestToThis >= 0 : nearestToThis + " is out of bounds.";		
	...
	public static int powerOfTwoExponent(int powerOfTwo) {
		...
		assert (powerOfTwo & 1) != 1 : powerOfTwo + " is not a power of two.";
		
		int iter = 0; 
		while((1 << iter) < powerOfTwo) iter++;
		
		assert (1 << iter) == powerOfTwo : powerOfTwo + " is not a power of two.";
	...
	public static int triangularNumber(int input) {
		...
		assert input >= 0 : "Triangular number inputs must be positive.";
	...
	public static int cantor(int x , int y) {
		...
		assert x >= 0 : "Cantor inputs must be positive.";
		assert y >= 0 : "Cantor inputs must be positive.";
		int sum = x + y;
