...
public class LHSPanel {
	...
	public LHSPanel(Editor editor , CSNuklear nuklear) {
		...
		Engine.THE_TEMPORAL.onTrue(() -> editor.currentProject() != null , () -> {

			CSDynamicRow threeDColorRow = ui.new CSDynamicRow(200);
			color = threeDColorRow.new CSColorPicker(editor.currentProject().channelsPerPixel() == 4 ? RGBA : RGB);
			...
			while(brushes.hasNext()) {
				...
				CSRadio newRadio = ui.new CSDynamicRow(25).new CSRadio(
					iter.getClass().getSimpleName().replace("Brush", "").replace('_', ' ') ,				
					false ,
					() -> editor.setBrushTo(finalIter)
				);
		...
		if(Engine.isDebug()) {
			...
			debugProjectRow.doLayout = () -> editor.currentProject() == null;
	...
	private int channels() {
		...
		return editor.currentProject().getChannelsPerPixelOfCurrentLayer();
