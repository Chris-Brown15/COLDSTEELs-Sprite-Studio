...
public final class Directory {
	...
	private static final CSCHashMap<Directory , String> roots = new CSCHashMap<>(7);
	
	public static synchronized final Directory establishRoot(final String rootName) {
	...
	public static synchronized final Directory getRoot(final String name) {
	...
	private final ConcurrentHashMap<String , Directory> subdirectories = new ConcurrentHashMap<>();
	 ...
	 */
	private Directory(final String name) {
	...
	public Directory createSubdirectory(String name) { 
		...
		Directory newSub = new Directory(name , this);
	 ...
	 */
	public Directory[] createSubdirectories(String...names) {
		
		Directory[] directories = new Directory[names.length];
	...
	public void seekExistingFiles() {
		...
		KeySetView<String, Directory> directoryNames = subdirectories.keySet();
		
		for(File x : realFiles) { 
			
			if(fileNames.contains(x.getName())) continue;
			addFile(new CSFile(this , x));
		...
		for(File x : realDirectories) {
			...
			Directory asDirectory = createSubdirectory(x.getName());
	 ...
	 * 	{@linkplain Directory#copy(Directory) copy}, and then deleted. Like with {@link Directory#delete() delete}, the object who called
	 ...
	 * 	subdirectories of it will appear after each other, separated by the {@link Directory#separator separator}.
	 * </p>
	 * 
	 ...
	 */
	public boolean isSubdirectory(Directory subdirectory) {
		
		if(subdirectories.containsKey(subdirectory.name)) return true;
		for(Directory sub : subdirectories.values()) if(sub.isSubdirectory(subdirectory)) return true;		
		return false;
	 ...
	 */
	public void deleteSubdirectory(Directory subdirectory) throws IOException {
	 ...
	 */
	public Directory copy(Directory copyInto) {
		
		Directory copy = new Directory(name , copyInto);
		
		for(CSFile file : files.values()) copy.createFile(file.name, file);
		
		for(Directory subdirectory : subdirectories.values()) subdirectory.copy(copy);
	...
	public void delete() throws IOException {
		...
		for(Directory directory : subdirectories.values()) directory.delete();
	...
	public Directory getSubdirectory(String name) {
		...
		Directory result = subdirectories.get(name);
	...
	public Iterator<Directory> subdirectories() {
		...
		return new Iterator<>() {
			...
			private final Iterator<Directory> sourceIter = subdirectories.values().iterator();

			private Directory next = null;
			...
			@Override public Directory next() {
	 ...
	 */
	public void forEachSubdirectoryPreorder(Consumer<Directory> callback) {
	 ...
	 * @param callback — callback for each subdirectory of this directory
	 */
	public void forEachSubdirectoryPostorder(Consumer<Directory> callback) {
